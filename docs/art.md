...
- **Скрипт** `tools/location-bg.py`: срезает чёрные кромки генератора (бывает полоса 10–35 px сверху или снизу — под ногами бойцов это чёрная лента), берёт кадр 3:1 по центру, уменьшает усреднением без сглаживания и квантует в 128 цветов (на глаз не отличается, файл втрое легче). Руками мастер не режем.

```bash
python tools/location-bg.py art/forest-bg.png --loc forest --zoom 1.6  --center 0.36   # фонарь у тропы
python tools/location-bg.py art/swamp-bg.png  --loc swamp  --zoom 1.55 --center 0.58   # настил с крестом
python tools/location-bg.py art/crypt-bg.png  --loc crypt  --zoom 1.6  --center 0.5    # ниша со статуей
python tools/location-bg.py art/hive-bg.png   --loc hive   --zoom 1.5  --center 0.5    # коконы у зева
python tools/location-bg.py art/caves-bg.png  --loc caves  --zoom 1.5  --center 0.3    # лавопад
python tools/location-bg.py art/ship-bg.png   --loc ship   --zoom 1.5  --center 0.52   # флаг, луна, пушки
```
