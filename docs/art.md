...
## Файлы и конвейер (сделано)

```
art/masters/<id>.png              мастер как отдал генератор: 1024 или лист на белом фоне
src/assets/sprites/<id>.png       рабочий кадр, который берёт игра (делает скрипт)
tools/art/prepare.py              обработка: фон → прозрачность, обрезка, кадр ступнями на нижнюю кромку
```

Лист из шести героев режется одной командой:

```bash
python3 tools/art/prepare.py art/masters/heroes.png --grid 3x2 \
    --ids warrior,mage,assassin,paladin,berserk,archer --sheet /tmp/check.png
```

Что делает скрипт:

- **Фон выбивает заливкой от краёв**, а не правилом «всё белое прозрачно»: светлое пятно внутри
  силуэта (белый плащ Паладина, светлый щит Воина) заливке недоступно — его отсекает тёмный контур.
  Порог `--light` по умолчанию 110: так вместе с белым уходит и мягкая серая тень под ногами,
  которую генератор дорисовывает вопреки чек-листу.
- **Масштабирует всех одним множителем** (самый высокий занимает кадр целиком), поэтому рост
  персонажей друг относительно друга сохраняется. `--per-cell` — если нужно растянуть каждого по своей клетке.
- **Ставит ступни на нижнюю кромку кадра.** В бою спрайт прижимается низом к земле, и лишние пустые
  пиксели под ногами подняли бы бойца в воздух над остальными.
- Уменьшает с премножением на альфу — иначе сглаживание подмешивает в кромку цвет прозрачных пикселей.
- `--sheet` собирает контрольную полосу результата, чтобы глазами проверить всех сразу.

Подключение в игре (`src/ui/art.ts`): файл `src/assets/sprites/<id>.png` подменяет процедурный спрайт
везде, где боец показывается, — бой, блок героя, выбор героя, лист персонажа, бестиарий, итоги.
Имя файла и есть id из `heroes.ts` / `enemies.ts`, никаких правок в данных не нужно; файла нет —
рисуется старый процедурный спрайт. Так HD-персонажи вводятся по одному, без остановки игры.

Размер рабочего кадра — `--size`, по умолчанию 128. На FullHD логический кадр 960×540 растянут ×2,
то есть 128-пиксельный герой показывается в 256 физических пикселей: пиксель остаётся видимым и
крупным. Кадр 256 дал бы на FullHD 1:1 и более мелкий пиксель — это вопрос вкуса, а не правильности.
`image-rendering: pixelated` остаётся: увеличение чёткое. А вот **уменьшение** (блок героя 44 px,
карточка выбора 64) `spriteImg` сглаживает — чёткое уменьшение выедает пиксели, тонкий клинок
исчезает целиком.
