...
// ─── Части карточек для прототипов v0.50 (variants.ts) ─────────────────────
// Общие кирпичи всех вариантов карточек: тир точками, параметры приёма с иконками (цена, перезарядка, цель, тип),
// описание без хвостов «КД 3» и «Раз в ход» — они переехали в параметры. Варианты различаются только раскладкой.

/** Тир точками: ●●○ — горят набранные, цвет — цвет тира. Словами тир — в подсказке. */
export function tierPips(tier: number, max: number, color: string, tip?: string): HTMLElement {
  return h(
    'span',
    { class: 'tier-pips', tip: tip ?? `Тир ${tier} из ${max}` },
    ...Array.from({ length: max }, (_, i) => h('i', { class: i < tier ? 'on' : '', style: i < tier ? `background:${color}` : '' })),
  );
}
