...
import { battleLogView } from '../logView';
...
function pastBattle(entry: BattleLog, open: boolean): HTMLElement {
  ...
  const fill = () => {
    if (details.open && !body.firstChild) body.appendChild(battleLogView(entry.lines, entry.marks));
  };
...
export function runLogBody(run: RunState): HTMLElement[] {
  ...
  return [...past, head, battleLogView(live.log, live.logMarks)];
}

/** Шапка лога: заголовок и крестик — одна и та же в панели боя и в оверлее. */
export function logHead(app: App, title: Child, cls: string): HTMLElement {
  return h('div', { class: cls }, title, button('✕', () => app.toggleLog(), { class: 'small' }));
}
