...
function wolfFigure(p: Painter, L: WolfLook): void {
  const M = L.M;
  // Укус: замах — присесть и отвести голову, выпад — бросок вперёд с раскрытой пастью.
  // Урон: отбросило назад, морда вверх в визге, уши прижаты, глаза зажмурены.
  const { wind, strike } = p.attack();
  const hurt = p.hurt();
  const breath = p.wave(L.alpha ? 3 : 4);
  const bob = p.snap(0.9 * p.wave(L.alpha ? 3 : 4, 0.15));
  const tail = p.wave(1);
  const pant = (p.wave(L.alpha ? 3 : 4, 0.05) + 1) / 2;
  const ex = p.blink(0.62, 0.06) ? 2.5 : 0;

  p.pose({ dx: -14 * strike + 4 * wind + 5 * hurt, rot: -0.05 * strike + 0.05 * wind + 0.07 * hurt, px: 88, py: 76 }, () => {
    p.shadow(64, 44, 3);
    ...
    // Хвост висит поленом и мерно качается (у вожака — напряжён), в атаке задран; верх и кончик темнее.
    const tx = (L.alpha ? 1.2 : 2.5) * tail;
    const lift = 6 * (wind + strike) - 2 * hurt;
    p.chain([[95, 35, 5], [104, 40 - lift * 0.3, 5.5], [109 + tx * 0.5 + lift * 0.4, 50 - lift, 5], [110 + tx + lift * 0.7, 60 - lift * 1.5, 4], [108 + tx * 1.3 + lift, 68 - lift * 2, 2]], M.fur, { part: 'tail' });
    p.chain([[109 + tx * 0.9 + lift * 0.8, 58 - lift * 1.4, 3.4], [108 + tx * 1.3 + lift, 68 - lift * 2, 1.8]], M.back, { part: 'tail', paint: true });
    p.chain([[96, 33, 3], [104, 37 - lift * 0.3, 3.2], [108 + tx * 0.5 + lift * 0.4, 45 - lift, 2.5]], M.back, { part: 'tail', paint: true });
    ...
    // Ближние лапы: на выпаде передняя тянется вперёд и отрывается от земли.
    const reach = 4 * strike;
    p.chain([[43, 46, 6.5], [45 - reach * 0.5, 57, 4.2], [43 - reach, 69 - reach * 0.6, 2.8], [42 - reach, 73 - reach * 0.6, 2.6]], M.fur, { part: 'near' });
    p.ellipse(40 - reach, 74 - reach * 0.6, 4.4, 2.3, M.fur, { part: 'near' });
    p.chain([[81, 43, 8], [85, 55, 5], [90, 65, 3], [87, 73, 2.7]], M.fur, { part: 'near' });
    p.ellipse(85.5, 74, 4.4, 2.3, M.fur, { part: 'near' });
    p.px(36.5 - reach, 74.5 - reach * 0.6, '#2a2630');
    p.px(39 - reach, 75 - reach * 0.6, '#2a2630');
    if (L.alpha) {
    ...
    const hx = p.snap(-3 * strike + 1.5 * wind + hurt);
    const hy = bob + p.snap(2 * wind + strike - 3 * hurt);
    p.limb(45, 36, 12, 31 + hx, 30 + hy, 10, M.fur);
    p.ellipse(36, 43, 7.5, 9.5, M.pale, { paint: true });
    const tufts = L.alpha ? 7 : 4;
    const raise = L.alpha ? 1.2 * (breath + 1) + 1.5 * wind : wind;
    for (let k = 0; k < tufts; k++) {
      const x = 36 + k * 6, y = 23 + k * 1.8 + (k < 1 ? hy : 0) - (L.alpha ? 3 - k * 0.3 : 0) - raise;
      p.poly([x - 3, y + 7, x + 2, y - 2 - (k % 2), x + 4.5, y + 7], M.back, { bevel: 1.5 });
    }
    // Грива вожака: густой воротник вокруг шеи, лохматый край.
    if (L.alpha) p.ellipse(35 + hx * 0.5, 36 + hy * 0.5, 11, 12.5, M.mane, { lift: 2 });

    // Голова в профиль: клин черепа, «стоп» у глаз, длинная прямая морда, ухо торчком, щёки в густой шерсти.
    p.pose({ dx: hx, dy: hy, rot: -0.08 * strike + 0.28 * hurt, px: 31, py: 30 }, () => {
      const back = 4 * hurt;
      p.poly([27, 23, 31 + ex * 0.6 + back, 9 + back * 0.7, 36, 23], M.back, { part: 'ear', bevel: 2, tone: -0.1 });
      const jaw = (L.alpha ? 2.2 + 1.2 * pant : 0.5 + 1.6 * pant) + 4.5 * strike + 2.5 * hurt;
      p.limb(19, 35.5, 3.6, 6, 36 + jaw, 2.1, M.pale, { part: 'jaw', tone: -0.15 });
      p.ellipse(25, 27, 10.5, 8.5, M.fur, { part: 'head' });
      p.limb(21, 29.5, 6, 4, 31.5, 3.6, M.fur, { part: 'head' });
      p.poly([19, 22, 25 + ex + back, 6 + back * 0.8, 31, 21], M.fur, { part: 'head', bevel: 2.5 });
      p.poly([22.5, 19, 25 + ex + back, 10 + back * 0.8, 28, 19], M.ear, { part: 'head', paint: true });
      if (L.alpha) p.erase(29 + ex * 0.5 + back, 13 + back * 0.7, 1.6, 1.6);
      p.limb(20, 28, 3, 6, 29.5, 1.8, M.pale, { part: 'head', paint: true, tone: 0.05 });
      p.ellipse(27, 33.5, 7.5, 5, M.pale, { part: 'head', paint: true });
      p.ellipse(2.8, 31, 2.4, 2.1, M.nose, { part: 'head', lift: 3 });
      p.line(6, 34.5, 18, 35, '#1c141a');
      if (L.alpha || strike > 0.3) {
        // Оскал: губа задрана, клыки наружу (у вожака всегда, у волка — в укусе).
        p.poly([6.5, 34.5, 8.5, 34.5, 7.8, 38.5], M.fang, { part: 'fang', bevel: 0.6 });
        p.poly([11, 36 + jaw, 13, 36 + jaw, 12, 32.5 + jaw * 0.5], M.fang, { part: 'fang', bevel: 0.6 });
      } else {
        p.px(7, 35.5, '#efe6d6');
        p.px(13, 36, '#efe6d6');
      }
      if (L.alpha) {
        // Морщины на переносице и шрам через морду.
        p.line(9, 28.5, 12, 30, '#23222e');
        p.line(12.5, 28, 15, 29.5, '#23222e');
        p.line(16, 23, 21, 31, '#a8a2b8');
        p.glow(17.5, 26, 4, L.eye, 0.35);
      }
      // Глаз под тяжёлой бровью, бровь скошена к носу — зверь смотрит исподлобья.
      p.eye(17.5, 26, 1.5, L.eye, { closed: hurt > 0.4 ? 1 : p.blink(0.3), glint: L.glint });
      p.line(14, 24, 21, 23, '#2a2630');
    });
  });
}
...
const boarBase: Model = {
  ...
  draw(p: Painter) {
    const M = BOAR;
    const G = 70;
    // Таран: замах — осесть на задние, голову к земле; выпад — рывок вперёд, клыки поддевают снизу вверх.
    // Урон: отбросило, голова вскинута в визге, глаза зажмурены.
    const { wind, strike } = p.attack();
    const hurt = p.hurt();
    ...
    p.pose({ dx: -14 * strike + 4 * wind + 5 * hurt, rot: -0.04 * strike + 0.04 * wind + 0.06 * hurt, px: 92, py: G }, () => {
      p.shadow(64, 48, 3);
      ...
      // Хвостик-плеть с кисточкой: изредка дёргается, в атаке и от удара задран.
      const tf = flick ? 3 : 3 * Math.max(wind, strike, hurt);
      ...
      // Щетина по хребту: выше всего на горбу, поднимается с каждым вдохом и дыбом перед рывком.
      const bristle = 0.8 * (breath + 1) + 2.5 * wind;
      for (let k = 0; k < 9; k++) {
        const x = 30 + k * 6.5;
      ...
      // Ближние ноги: на рывке передняя выброшена вперёд.
      const sx = 3 * scrape - 4 * strike, sy = 2.5 * scrape + 2 * strike;
      ...
      // Голова клином к земле: ухо назад, пятак, клыки из нижней челюсти. На выпаде морда поддевает вверх.
      p.pose({ dx: p.snap(-3 * strike), dy: hy + p.snap(3 * wind - hurt), rot: -0.14 * wind + 0.22 * strike + 0.22 * hurt, px: 36, py: 40 }, () => {
        const back = 3 * hurt;
        p.poly([31, 34, 37 + back, 23 + back, 41, 33], M.fur, { part: 'ear', bevel: 1.8, tone: -0.05 });
        p.poly([34, 32, 37 + back, 26 + back, 39, 32], M.mane, { part: 'ear', paint: true });
        const open = 1.5 * strike + 3 * hurt;
        p.limb(23, 54, 4.5, 11, 57.5 + open, 3.2, M.fur, { part: 'jaw', tone: -0.15 });
        p.ellipse(27, 42, 13, 10.5, M.fur, { part: 'head', rot: 0.35 });
        const sn = sniff * (1 - Math.max(wind, strike, hurt));
        p.limb(19, 47, 7.5, 7, 52.5 + sn, 5.2, M.fur, { part: 'head' });
        p.ellipse(4.5, 53 + sn, 2.5, 4.6, M.snout, { part: 'head', lift: 3 });
        p.px(3.5, 51.5 + sn, '#2a1414');
        p.px(3.5, 54.5 + sn, '#2a1414');
        p.chain([[13, 57 + open, 1.9], [10, 52 + open, 1.5], [10, 47 + open, 0.9]], M.tusk, { part: 'tusk' });
        p.chain([[17, 57 + open, 1.4], [15, 53 + open, 1.1], [15.5, 50 + open, 0.7]], M.tusk, { part: 'tusk2', tone: -0.2 });
        p.line(9, 55.5, 20, 55, '#2a1414');
        p.eye(22, 40.5, 1.3, '#ff8a3d', { closed: hurt > 0.4 ? 1 : p.blink(0.7), glint: '#ffe0c0' });
        p.line(18.5, 38, 25, 39.5 + 1.5 * wind, '#1e1512');
      });
    });
  },
};
...
export const rat: Model = {
  ...
  draw(p: Painter) {
    ...
    const sniff = 0.6 * p.wave(9) * (1 - Math.max(wind, strike, hurt));
    const hy = p.snap(0.5 * p.wave(3, 0.1));
    const sw = p.wave(1), sw2 = p.wave(1, 0.2);
    const ear = p.blink(0.42, 0.06);

    p.pose({ dx: -9 * strike + 3 * wind + 4 * hurt, rot: -0.06 * strike + 0.05 * wind + 0.08 * hurt, px: 52, py: 42 }, () => {
      p.shadow(42, 30, 2.2);
      // Голый хвост лежит кольцом по земле и лениво пошевеливает кончиком; от удара кончик взлетает.
      const curl = 5 * hurt + 2 * wind;
      p.chain([[60, 31, 2.2], [69, 34, 1.8], [74, 39, 1.5], [70 + sw, 41.5, 1.2], [60 + sw2 * 2, 41.5 - curl * 0.4, 0.9], [52 + sw2 * 3, 40.5 - sw * 0.5 - curl, 0.6]], M.skin, { part: 'tail' });
      ...
      // Ближние лапы: задняя — длинная ступня, передняя — ручка у груди, в броске тянется вперёд.
      const reach = 3 * strike;
      p.limb(53, 35, 4.5, 50, 40, 2.4, M.fur, { part: 'near' });
      p.limb(46, 41, 1.3, 55, 41.5, 1.2, M.skin, { part: 'near' });
      p.limb(26, 36, 2.3, 24 - reach, 40 - reach * 0.5, 1.6, M.fur, { part: 'near' });
      p.limb(21.5 - reach, 41 - reach * 0.5, 1.1, 26 - reach, 41.2 - reach * 0.5, 1.1, M.skin, { part: 'near' });

      // Голова: острая морда, розовый нос, круглое ухо.
      p.pose({ dx: p.snap(-3 * strike + wind + hurt), dy: hy + p.snap(-2 * wind + strike - 2 * hurt), rot: 0.2 * wind - 0.1 * strike + 0.3 * hurt, px: 24, py: 30 }, () => {
        const back = 2 * hurt;
        p.ellipse(20, 30, 8.5, 6.5, M.fur, { part: 'head', rot: 0.2 });
        p.limb(16, 32, 4.5, 5 + sniff * 0.5, 35 + sniff, 1.8, M.fur, { part: 'head' });
        p.ellipse(18, 34.5, 5, 2.2, M.belly, { part: 'head', paint: true });
        p.disc(4 + sniff * 0.5, 35 + sniff, 1.1, '#d49a94');
        p.ellipse(25.5 + back, 24 - ear + back, 3.4, 3.8, M.skin, { part: 'ear', rot: 0.3 });
        p.ellipse(25.2 + back, 24.6 - ear + back, 2, 2.4, M.earIn, { part: 'ear', paint: true, rot: 0.3 });
        // Резцы: в броске и в писке пасть открыта.
        if (strike > 0.3 || hurt > 0.4) {
          p.line(6, 37, 12, 37.5, '#2a1414');
          p.block(6, 37, 1, 2, '#f0e2c4');
          p.block(7.5, 35.5, 1, 2, '#f0e2c4');
        } else p.px(6.5, 37 + sniff, '#f0e2c4');
        if (p.d < 3) {
          const wy = 34 + sniff;
          p.line(8, wy, 1, wy - 3 - sniff, '#bdb4a4');
          p.line(8, wy + 1, 0, wy + 1.5, '#bdb4a4');
        }
        p.eye(15.5, 29, 1.3, '#ff5555', { closed: hurt > 0.4 ? 1 : p.blink(0.75), glint: '#ffd0d0' });
      });
    });
  },
};
...
export const bat: Model = {
  ...
  draw(p: Painter) {
    ...
    // Урон: отбросило вверх-назад, крылья вскинуты, глаза зажмурены.
    const { wind, strike } = p.attack();
    const hurt = p.hurt();
    ...
    p.pose({ dx: -16 * strike + 3 * wind + 6 * hurt, dy: -6 * wind + 9 * strike - 4 * hurt, rot: -0.2 * strike + 0.25 * hurt, px: cx, py: 30 }, () => {
      ...
      if (strike > 0.3 || hurt > 0.4) {
        // Пасть раскрыта: клыки вниз.
        p.block(cx - 2, 21.5 + by, 3, 1, '#2a1420');
...
export const spider: Model = {
  ...
  draw(p: Painter) {
    const M = SPIDER;
    // Укус: замах — встать на дыбы, передние лапы подняты; выпад — бросок вперёд, хелицеры с клыками выброшены.
    // Урон: отбросило назад, лапы поджаты к телу.
    const { wind, strike } = p.attack();
    const hurt = p.hurt();
    ...
    p.pose({ dx: -10 * strike + 3 * wind + 4 * hurt, rot: 0.14 * wind + 0.05 * hurt, px: 84, py: 51 }, () => {
      p.shadow(52, 46, 3);
      ...
      const leg = ([rx, ry, kx, ky, ax, ay, fx, fy]: Leg, i: number, part: string, tone: number, r0: number, front = 0): void => {
        const r = r0 * thin;
        const lift = 2.5 * p.blink(0.12 + i * 0.22, 0.1) + front * (14 * wind);
        const fwd = front * (-5 * wind - 4 * strike);
        // Поджать: стопы тянутся к телу.
        const tuck = 0.18 * hurt;
        const fx2 = fx + (52 - fx) * tuck + fwd, ax2 = ax + (52 - ax) * tuck * 0.6 + fwd * 0.6;
        const mat = p.d >= 2 ? (part === 'near' ? M.legLine : M.legFar) : M.leg;
        p.chain([[rx, ry + by, 1.9 * r], [kx + fwd * 0.3, ky + by * 0.6 - lift * 0.3, 1.4 * r], [ax2, ay - lift, 1 * r], [fx2, fy - lift * 1.2, 0.6 * r]], mat, { part, tone });
        // Светлый сустав на колене: без него нога сливается с соседней.
        if (part === 'near') p.disc(kx + fwd * 0.3, ky + by * 0.6 - 0.5 - lift * 0.3, 0.9, '#6a6072');
      };
      SPIDER_FAR.forEach((l, i) => leg(l, (i + 2) % 4, 'far', -0.3, 0.8, i === 0 ? 0.7 : 0));
      ...
      SPIDER_NEAR.forEach((l, i) => leg(l, i, 'near', 0.05, 1, i === 0 ? 1 : i === 1 ? 0.5 : 0));

      // Хелицеры с клыками, педипальпы подёргиваются, гроздь красных глаз.
      const jab = 4 * strike;
      p.limb(29, 36 + by, 2.4, 27 - jab, 42 + by + jab * 0.3, 1.5, M.chitin, { part: 'fang' });
      p.limb(32, 37 + by, 2.2, 31 - jab, 42.5 + by + jab * 0.3, 1.4, M.chitin, { part: 'fang', tone: -0.15 });
      p.chain([[27 - jab, 42 + by + jab * 0.3, 1.2], [26 - jab * 1.3, 45 + by + jab * 0.2, 0.8]], M.fang, { part: 'fang' });
      p.chain([[30, 34 + by, 1.3], [22 - 2 * wind, 37 + by + palp - 3 * wind, 1.1], [20 + palp - 3 * wind, 42 + by - 4 * wind, 0.9]], M.hair, { part: 'palp' });
...
export const banditArcher: Model = {
  ...
  draw(p: Painter) {
    const M = ARCHER;
    const G = 110;
    // Выстрел: замах — натянуть тетиву к уху, выпад — спуск: тетива прямая, стрела ушла к герою, кисть отлетела назад.
    // Урон: отбросило, голова назад, лук опущен, глаза в тени погасли.
    const { wind, strike } = p.attack();
    const hurt = p.hurt();
    ...
    p.pose({ dx: 2 * strike + 4 * hurt, rot: 0.02 * wind + 0.06 * hurt, px: 38, py: G }, () => {
      p.shadow(38, 24, 3);
      ...
      p.pose({ dx: p.snap(1.5 * hurt), dy: p.snap(-hurt), rot: 0.1 * hurt, px: 40, py: 32 }, () => {
        ...
        if (!p.blink(0.55, 0.05) && hurt < 0.4) {
          p.px(29, 16.5 + up, '#e8dcb0');
          p.px(33.5, 16.5 + up, '#e8dcb0');
        }
      });

      // Лук в вытянутой руке, стрела на тетиве смотрит на героя. При натяжении лук чуть выше, от удара опущен.
      const bx = aim * 0.5, byy = aim - 2 * wind + 3 * hurt;
      ...
      // Тянущая кисть: к уху на натяжении, после спуска отлетает дальше назад.
      const loaded = strike < 0.3;
      const hx = 31 + 13 * wind + 18 * strike, hy = 56 + byy * 0.5 - 2 * wind - 4 * strike;
      if (loaded) {
        p.line(21.5, 28 + up + byy, hx, hy, '#d8d0c0');
        p.line(hx, hy, 21.5, 84 + byy, '#d8d0c0');
        const tip = 3 + 12 * wind;
        p.line(hx, hy, tip + 5, hy, '#8a6a4a');
        p.poly([tip, hy, tip + 5.5, hy - 2.4, tip + 5.5, hy + 2.4], M.metal, { part: 'arrow', bevel: 1 });
        p.px(hx - 1, hy - 1, '#c83a32');
      } else p.line(21.5, 28 + up + byy, 21.5, 84 + byy, '#d8d0c0');
      // Стрела в полёте: в кадрах спуска уходит влево, к герою.
      if (p.clip === 'attack' && p.u > 0.4 && p.u < 0.6) {
        const tip = p.u < 0.5 ? -6 : -24;
        const ay = 55 + byy * 0.5;
        p.line(tip + 5, ay, tip + 18, ay, '#8a6a4a');
        p.poly([tip, ay, tip + 5.5, ay - 2.4, tip + 5.5, ay + 2.4], M.metal, { part: 'flyArrow', bevel: 1 });
        p.line(tip + 20, ay, tip + 30, ay, '#e8e0c8a0');
      }
...
/** Рубка тесаком: рука из покоя уходит назад-вверх за голову, дугой через верх обрушивается вперёд-вниз и возвращается. */
const CHOP: Keys = [[0, -0.8], [0.14, -2.2], [0.3, -2.45], [0.43, -3.6], [0.57, -5.35], [0.72, -5.75], [0.86, -6.1], [1, -2 * Math.PI]];
...
export const cutthroat: Model = {
  ...
  draw(p: Painter) {
    const M = CUT;
    const G = 114;
    // Удар: тесак заносится из-за головы и рубит вперёд-вниз, корпус подаётся за ним. Урон: отбросило, голова назад.
    const { wind, strike } = p.attack();
    const hurt = p.hurt();
    const up = -p.bob(1.5, 2);
    const heave = (p.wave(2) + 1) / 2;

    p.pose({ dx: -7 * strike + 2 * wind + 4 * hurt, rot: 0.04 * wind - 0.06 * strike + 0.05 * hurt, px: 42, py: G }, () => {
      p.shadow(42, 30, 3.5);
      ...
      p.pose({ dx: p.snap(1.5 * hurt), rot: 0.14 * hurt - 0.05 * strike, px: 43, py: 32 + up }, () => {
        const knot = 1.5 * p.wave(2, 0.3) + 3 * hurt - 2 * wind;
        ...
        // Рот: в замахе оскал, от удара раскрыт.
        if (wind > 0.5 || strike > 0.5 || hurt > 0.4) {
          p.line(30, 26.5 + up, 35.5, 27 + up, '#140e0c');
          p.line(30.5, 27.5 + up, 35, 28 + up, hurt > 0.4 ? '#5a2020' : '#e8e0d0');
        } else p.line(30, 26.5 + up, 35.5, 27 + up, '#140e0c');
        p.line(30.5, 15 + up, 37, 16.5 + up + 1.5 * wind, '#241a14');
        p.eye(34, 18.5 + up, 1.1, '#e8e0d0', { closed: hurt > 0.4 ? 1 : p.blink(0.42), pupil: '#1a1210' });
        p.line(35.5, 14.5 + up, 32.5, 22 + up, '#9a5a44');
        p.px(46.5, 25 + up, '#e8c060');
      });

      // Ближняя рука с тесаком: в покое лезвие чуть покачивается и по кромке пробегает блик; в ударе рука идёт дугой.
      p.pose({ rot: arc(p, CHOP), px: 26, py: 38 + up }, () => {
        ...
        // Блик бежит по кромке в начале цикла, но не в самом первом кадре: им же кончаются клипы удара и урона.
        const g = p.clip === 'idle' && p.t > 0.03 && p.t < 0.33 ? (p.t - 0.03) / 0.3 : -1;
        if (g >= 0) {
...
export const goblin: Model = {
  ...
  draw(p: Painter) {
    const M = GOB;
    // Подлый удар: замах — клинок отведён назад, выпад — укол вперёд поверх щита. Урон: сжался за щитом, уши прижаты.
    const { wind, strike } = p.attack();
    const hurt = p.hurt();
    // Пружинит на полусогнутых: верх тела вниз-вверх, ступни стоят.
    const by = p.bob(1.5, 3);
    const ear = 2 * p.wave(3, 0.2) + 4 * hurt;
    const wig = 1.2 * p.wave(3, 0.5);

    p.pose({ dx: -8 * strike + 2 * wind + 3 * hurt, rot: -0.05 * strike + 0.03 * wind + 0.06 * hurt, px: 36, py: 74 }, () => {
      p.shadow(36, 22, 2.5);
      ...
      // Дальняя рука с ржавым клинком занесена над головой; в уколе выходит вперёд, поверх головы и щита.
      const arm = (): void => {
        p.pose({ rot: 0.5 * wind - 1.9 * strike - 0.25 * hurt, px: 46, py: 37 + by }, () => {
          p.chain([[46, 37 + by, 3.4], [55, 31 + by, 3], [58, 21 + by, 2.8]], M.skin, { part: 'farArm', tone: strike > 0.2 ? 0 : -0.12 });
          ...
          p.ellipse(58, 22 + by, 2.8, 2.6, M.skin, { part: 'fist', tone: strike > 0.2 ? 0 : -0.1 });
        });
      };
      const armFront = strike > 0.2;
      if (!armFront) arm();
      ...
      // Уши-лопухи: дальнее за головой, ближнее поверх. От удара голова втягивается за щит.
      p.pose({ dy: p.snap(2.5 * hurt), dx: p.snap(-1.5 * strike), rot: 0.1 * hurt, px: 34, py: 28 + by }, () => {
        p.poly([26, 14 + by, 11 + ear * 0.8, 5 + by - ear * 0.5 + ear, 23, 20 + by], M.skin, { part: 'ear2', bevel: 2, tone: -0.1 });
        p.ellipse(32, 20 + by, 12, 10.5, M.skin, { part: 'head' });
        p.poly([40, 15 + by, 64 - ear, 8 + by + ear, 42, 24 + by], M.skin, { part: 'ear', bevel: 2.5 });
        p.poly([43, 17 + by, 58 - ear, 11 + by + ear, 44, 21 + by], M.skinDark, { part: 'ear', paint: true });
        ...
        const shut = hurt > 0.4 ? 1 : p.blink(0.8);
        ...
        p.line(18.5, 13.5 + by, 24, 15 + by + 1.5 * wind, '#23301c');
        p.line(27, 14 + by + 1.5 * wind, 33, 13 + by, '#23301c');
      });

      // Круглый щит в ближней руке: доски, железный обод и умбон. От удара вскинут выше.
      const sy = by - p.snap(3 * hurt);
      p.limb(33, 38 + by, 3.8, 24, 46 + sy, 3.2, M.skin, { part: 'nearArm' });
      p.ellipse(18, 48 + sy, 10.5, 12.5, M.iron, { part: 'shield', flat: 0.6 });
      p.ellipse(18, 48 + sy, 9, 11, M.wood, { part: 'shield', flat: 0.6, lift: 1 });
      p.ellipse(18, 48 + sy, 3, 3.2, M.iron, { part: 'shield', lift: 4 });
      if (p.d < 3) {
        p.line(13, 38 + sy, 13, 58 + sy, '#4a3422');
        p.line(23, 38 + sy, 23, 58 + sy, '#4a3422');
      }
      if (armFront) arm();
    });
  },
...
export const goblinShaman: Model = {
  ...
  draw(p: Painter) {
    const M = SHAMAN;
    const G = 80;
    // Искра: замах — посох вскинут, кристалл разгорается; выпад — посох выброшен вперёд, вспышка и искры веером.
    // Урон: отбросило, шляпа съехала назад, глаза зажмурены.
    const { wind, strike } = p.attack();
    const hurt = p.hurt();
    const hy = p.bob(1.2, 2);
    const hem = 1.6 * p.wave(1);
    const tip = 1.5 * p.wave(1, 0.3) + 3 * hurt;
    const pulse = (p.wave(2) + 1) / 2;

    p.pose({ dx: -4 * strike + 4 * hurt, rot: 0.03 * wind - 0.04 * strike + 0.05 * hurt, px: 36, py: G }, () => {
      p.shadow(38, 24, 3);
      ...
      p.pose({ dx: p.snap(hurt), rot: 0.12 * hurt, px: 32, py: 36 + hy }, () => {
        ...
        p.line(18, 36.5 + hy, 26, 37.5 + hy, strike > 0.5 ? '#3a1a1a' : '#1c2414');
        p.px(21, 37.5 + hy, '#e8e0c0');

        // Шляпа: поля, конус, заломленный кончик, перо. От удара съезжает назад.
        p.pose({ rot: 0.18 * hurt, px: 33, py: 20 + hy }, () => {
      ...
      // Посох с кристаллом: свечение дышит, вокруг кружат искры. В атаке посох ходит вокруг кисти.
      p.pose({ dy: -5 * wind, rot: 0.15 * wind - 0.5 * strike + 0.12 * hurt, px: 15.5, py: 47 + hy }, () => {
        ...
        const charge = Math.max(wind, strike);
        p.glow(15, 6, 7 + 3 * pulse + 5 * charge, '#e8d24a', 0.25 + 0.15 * pulse + 0.25 * charge);
        p.poly([15, 0, 19, 5, 15, 11, 11, 5], M.crystal, { part: 'crystal', bevel: 1.5 });
        const n = strike > 0.6 ? 8 : 3;
        const r = strike > 0.6 ? 14 : 10;
        for (let k = 0; k < n; k++) {
          const a = 2 * Math.PI * (p.t * 2 + k / n);
          p.px(15 + r * Math.cos(a), 6 + r * 0.6 * Math.sin(a), '#fff4a0');
        }
        if (strike > 0.6) p.disc(15, 5.5, 3, '#fffbe0');
      });
...
export const bear: Model = {
  ...
  draw(p: Painter) {
    const M = BEAR;
    const G = 144;
    // Удар лапой: замах — привстать на задние, передняя лапа занесена, пасть в рёве; выпад — навалиться вперёд,
    // лапа с когтями хлещет вниз. Урон: отбросило, голова вскинута, пасть открыта, глаза зажмурены.
    const { wind, strike } = p.attack();
    const hurt = p.hurt();
    ...
    p.pose({ dx: -14 * strike + 3 * wind + 7 * hurt, rot: 0.08 * wind - 0.02 * strike + 0.04 * hurt, px: 192, py: G }, () => {
      p.shadow(112, 96, 5);
      const paw = (x: number, part: string, tone = 0, lift = 0): void => {
        p.ellipse(x, G - 5 - lift, 15, 6, M.dark, { part, tone, flat: 0.3 });
        for (let k = 0; k < 3; k++) {
          const cx = x - 11 + k * 4.5;
          p.chain([[cx, G - 6 - lift, 1.5], [cx - 4, G - 3 - lift, 1.1], [cx - 5.5, G - 0.5 - lift, 0.6]], M.claw, { part: `${part}Claw`, tone });
        }
      };
      // Дальние лапы.
      ...
      // Ближние лапы: столбы с когтями. Передняя в замахе вскинута, в ударе выброшена вперёд.
      const lift = 22 * wind, fwd = -8 * wind - 16 * strike;
      p.chain([[80, 80, 20], [75 + fwd * 0.5, 110 - lift * 0.6, 15], [70 + fwd, 134 - lift, 12]], M.fur, { part: 'near' });
      paw(66 + fwd, 'near', 0, lift);
      p.chain([[176, 84, 24], [184, 110, 16], [176, 134, 12]], M.fur, { part: 'near' });
      paw(172, 'near');

      // Голова: круглые уши, широкий лоб, светлая морда, чёрный нос. В рёве и от удара пасть распахнута.
      p.pose({ dx: hx + p.snap(-5 * strike + 3 * hurt), dy: hy + p.snap(-5 * wind + strike - 4 * hurt), rot: 0.12 * wind - 0.04 * strike + 0.14 * hurt, px: 62, py: 62 }, () => {
        const open = 6 * wind + 5 * strike + 6 * hurt;
        p.ellipse(60, 34 - ear * 2 + 2 * hurt, 7.5, 7.5, M.dark, { part: 'ear', tone: -0.1 });
        p.limb(30, 81 + huff * 2 + open, 9.5, 16, 82 + huff * 4 + open * 1.4, 6.5, M.muzzle, { part: 'jaw', tone: -0.2 });
        p.ellipse(50, 58, 27, 23, M.fur, { part: 'head' });
        p.ellipse(44, 36, 7, 7, M.fur, { part: 'head', lift: 4 });
        p.ellipse(44, 36, 3.8, 3.8, M.dark, { part: 'head', paint: true });
        p.ellipse(40, 50, 15, 11, M.fur, { part: 'head', lift: 8 });
        p.limb(32, 68, 13, 17, 73, 9.5, M.muzzle, { part: 'head', lift: 4 });
        p.ellipse(10, 70, 6, 5, M.nose, { part: 'head', lift: 10 });
        p.line(12, 79, 28, 80 + huff, '#1a1010');
        if (open > 2) {
          p.line(15, 81 + open * 0.6, 26, 82 + open * 0.6, '#5a2020');
          p.block(14, 80, 1, 2, '#e8dcc0');
          p.block(19, 82 + open * 1.2, 1, 2, '#e8dcc0');
        } else if (huff > 0.5) p.line(15, 80.5 + huff * 2, 24, 81 + huff * 2, '#5a2020');
        p.eye(33, 56, 2.2, '#ffe8a3', { closed: hurt > 0.4 ? 1 : p.blink(0.45), glint: '#fffbe8' });
        p.line(27, 51 + 2 * wind, 38, 53, '#20150f');
      });
    });
  },
};
...
/**
 * Удар дубиной: рука с дубиной уходит назад за спину, через верх обрушивается вперёд, и дубина бьёт в землю
 * чуть впереди места, где лежала в покое (угол 0 — покой, полный оборот −2π — снова он).
 */
const SMASH: Keys = [[0, -0.9], [0.14, -2.3], [0.3, -2.6], [0.43, -4.2], [0.57, -6.13], [0.72, -6.13], [0.86, -6.2], [1, -2 * Math.PI]];
...
const trollBase: Model = {
  ...
  h: 162,
  ground: 158,
  // Дубина над головой в замахе поднимается выше горба на её длину.
  pad: 46,
  draw(p: Painter) {
    const M = TROLL;
    const G = 158;
    // Удар: дубина из-за спины через голову в землю, корпус валится вперёд, пасть в рёве. Урон: отбросило, голова назад.
    const { wind, strike } = p.attack();
    const hurt = p.hurt();
    const breath = p.wave(2);
    const up = -p.bob(2, 2);
    ...
    const drop = p.clip === 'idle' && p.t > 0.55 && p.t < 0.8 ? (p.t - 0.55) / 0.25 : -1;

    p.pose({ dx: -7 * strike + 2 * wind + 6 * hurt, rot: 0.05 * wind - 0.06 * strike + 0.04 * hurt, px: 70, py: G }, () => {
      p.shadow(78, 62, 5);
      ...
      // Ближняя рука до колен, в кулаке дубина с гвоздями, упёртая в землю; в ударе ходит дугой вокруг плеча.
      const phi = arc(p, SMASH);
      p.pose({ rot: phi, px: 62, py: 62 + up }, () => {
      ...
      // Пыль из-под дубины в кадрах удара о землю.
      if (strike > 0.6) {
        const c = Math.cos(phi), sn = Math.sin(phi);
        const dx = 12 - 62, dy = 150 - (62 + up);
        const cx = 62 + dx * c - dy * sn;
        for (const [ox, r] of [[-12, 3.5], [-4, 4.5], [6, 3.5], [13, 2.5]]) p.disc(cx + ox * strike, G - 3 - r * 0.4, r * strike, '#8a7a5ab0', true);
      }

      // Голова вжата в плечи и выдвинута вперёд: ухо, тяжёлое надбровье, нос-картошка, выпирающая челюсть с клыками.
      p.pose({ dx: p.snap(-3 * strike + 2 * hurt), dy: hy + p.snap(-2 * wind - 3 * hurt), rot: 0.06 * wind - 0.05 * strike + 0.12 * hurt, px: 48, py: 56 }, () => {
        const open = 3 * wind + 4 * strike + 4 * hurt;
        p.poly([54, 34, 72, 22, 64, 44], M.skin, { part: 'ear', bevel: 2, tone: -0.1 });
        p.ellipse(36, 63 + open, 17, 10, M.skin, { part: 'jaw' });
        p.ellipse(41, 44, 21, 18, M.skin, { part: 'head' });
        p.ellipse(35, 34, 16, 6, M.skin, { part: 'head', lift: 6 });
        p.ellipse(40, 28, 14, 4.5, M.moss, { part: 'head', paint: true, tone: -0.1 });
        p.limb(27, 43, 8.5, 14, 56, 7.5, M.skin, { part: 'nose', lift: 4 });
        p.poly([22, 64 + open, 26, 64 + open, 25.5, 51 + open], M.tusk, { part: 'tusk', bevel: 1 });
        p.poly([37, 65.5 + open, 41, 65.5 + open, 40, 53.5 + open], M.tusk, { part: 'tusk', bevel: 1 });
        if (open > 1.5) p.poly([22, 59, 46, 60, 45, 60 + open, 23, 59 + open], M.cap, { part: 'maw', tone: -0.4, bevel: 1 });
        else p.line(21, 61, 47, 62.5, '#1a2012');
        const shut = hurt > 0.4 ? 1 : p.blink(0.35);
        p.eye(26, 39, 2.2, '#ffe8a3', { closed: shut, glint: '#fffbe8' });
        p.eye(38, 38.5, 2.2, '#ffe8a3', { closed: shut, glint: '#fffbe8' });
        p.line(20, 35.5 + 2 * wind, 30, 37.5, '#1e2616');
        p.line(34, 36.5, 44, 35 + 2 * wind, '#1e2616');
        if (drop >= 0) p.disc(28, 67 + drop * (G - 71), 0.9, '#a8c8a0');
      });
    });
  },
};
...
export const gnomeThief: Model = {
  ...
  draw(p: Painter) {
    const M = GNOME;
    const G = 67;
    // Кража: замах — пригнуться и отвести руку, выпад — метнуться вперёд, рука хватает (или швыряет песок).
    // Урон: отбросило, колпак сбился, мешок подскочил, глаза зажмурены.
    const { wind, strike } = p.attack();
    const hurt = p.hurt();
    const by = p.bob(1, 3) + p.snap(1.5 * wind);
    // Глаза бегают: то на героя, то через плечо на мешок.
    const look = p.clip !== 'idle' ? -0.8 : p.t < 0.3 ? -0.8 : p.t < 0.6 ? 0.9 : -0.3;

    p.pose({ dx: -10 * strike + 2 * wind + 4 * hurt, rot: -0.08 * strike + 0.05 * wind + 0.08 * hurt, px: 32, py: G }, () => {
      p.shadow(38, 22, 2.5);

      // Мешок с золотом за спиной, из горловины блестят монеты; от удара подскакивает.
      const jolt = p.snap(-2.5 * hurt);
      p.ellipse(55, 40 + by + jolt, 14, 15, M.sack, { part: 'sack' });
      p.limb(45, 26 + by + jolt, 3, 49, 31 + by + jolt, 4.2, M.sack, { part: 'sack' });
      p.limb(46, 29 + by + jolt, 4.4, 47.5, 30.5 + by + jolt, 4.4, M.rope, { part: 'sack', paint: true });
      for (const [x, y] of [[42, 24], [45.5, 22], [48, 25]]) p.ellipse(x, y + by + jolt, 2.2, 1.8, M.gold, { part: 'coins' });
      p.ellipse(43, 29 + by + jolt, 3, 2.8, M.skin, { part: 'farHand', tone: -0.1 });
      ...
      p.line(22, 24.5 + by + wind, 26, 24 + by, '#e8e2d6');
      p.line(28.5, 24 + by, 32, 23.5 + by + wind, '#e8e2d6');
      p.poly([19.5, 24 + by, 42, 24 + by, 36, 8 + by, 31, 4 + by], M.cap, { part: 'cap', bevel: 3 });
      p.chain([[32, 6 + by, 2.6], [40, 2 + by + flop * 0.4, 1.8], [46.5, 6 + by + flop, 1]], M.cap, { part: 'cap' });
      p.ellipse(31, 24.5 + by, 12, 2.6, M.cap, { part: 'cap', tone: -0.2, flat: 0.5 });

      // Ближняя рука: в покое ладонью вверх подбрасывает монету (вертится ребром и плашмя), в атаке хватает.
      const [ex, ey] = mixPt([20, 44], [24, 47], [12, 40], wind, strike);
      const [hx, hy] = mixPt([15.5, 41], [23, 44], [3, 37], wind, strike);
      p.chain([[26, 38 + by, 3.2], [ex, ey + by, 2.8], [hx, hy + by, 2.6]], M.jacket, { part: 'nearArm' });
      p.ellipse(hx - 1, hy - 1 + by, 2.8, 2.3, M.skin, { part: 'hand' });
      if (p.clip === 'idle') {
        const u = (p.t - 0.1) / 0.4;
        const fly = u > 0 && u < 1;
      ...
      } else if (strike < 0.5) p.ellipse(hx - 1, hy - 5 + by, 2.2, 1.9, M.gold, { part: 'coin' });
    });
  },
};
...
const snatcherBase: Model = {
  ...
  draw(p: Painter) {
    const M = SNATCH;
    const G = 67;
    // Кража: замах — пригнуться и отвести руку с амулетом, выпад — метнуться вперёд и цапнуть; амулет мотается.
    // Урон: отбросило, капюшон сбился, глаза в тени погасли.
    const { wind, strike } = p.attack();
    const hurt = p.hurt();
    const by = p.bob(1, 3, 0.2) + p.snap(1.5 * wind);
    const sway = 1.5 * p.wave(1) + 3 * hurt;
    // Украденный амулет качается маятником на цепочке.
    const swing = Math.max(-1.6, Math.min(1.6, p.wave(2) + 1.5 * wind - 2.5 * strike + 1.2 * hurt));
    ...
    p.pose({ dx: -10 * strike + 2 * wind + 4 * hurt, rot: -0.08 * strike + 0.05 * wind + 0.08 * hurt, px: 32, py: G }, () => {
      ...
      p.poly([32, 18 + by, 50 + sway, 14 + by + 2 * hurt, 41, 28 + by], M.hood, { part: 'hood', bevel: 2 });
      ...
      if (!p.blink(0.62) && hurt < 0.4) {
        p.px(23.5, 27 + by, '#d8e8ff');
        p.px(28, 26.5 + by, '#d8e8ff');
      }

      // Ближняя рука поднята: на пальцах цепочка, внизу качается амулет. В атаке рука выброшена вперёд.
      const [ex, ey] = mixPt([20, 40], [24, 44], [12, 38], wind, strike);
      const [hx, hy] = mixPt([15, 33], [22, 38], [3, 33], wind, strike);
      p.chain([[27, 37 + by, 3.2], [ex, ey + by, 2.8], [hx, hy + by, 2.4]], M.coat, { part: 'nearArm' });
      p.ellipse(hx - 0.5, hy - 1.5 + by, 2.5, 2.3, M.skin, { part: 'hand' });
      const ax = hx - 0.5 + 4 * swing, ay = hy + 11 + by - Math.abs(swing) * 1.2;
      p.line(hx - 0.5, hy + by, ax, ay - 2, '#b8b8c8');
