    ...
    p.pose({ dx: -4 * strike + 2 * wind + 5 * hurt, rot: 0.03 * wind - 0.04 * strike + 0.05 * hurt, px: 46, py: G }, () => {
      ...
      p.pose({ dy: 2, rot: -0.08, px: 46, py: 80 }, () => {
        ...
        p.pose({ dx: p.snap(1.5 * hurt), dy: p.snap(2 - 2 * hurt), rot: -0.1 + 0.24 * hurt - 0.04 * strike, px: 44, py: 46 + up }, () => p.scope(1.1, 44 * -0.1, (46 + up) * -0.1, () => {
          ...
          p.poly([24, 46 + up, 25, 34 + up, 31, 22 + up, 40, 15 + up, 50, 12 + up, 60, 13.5 + up, 77 + tip, 26 + up + tip, 67, 31 + up, 66, 44 + up, 60, 50 + up, 46, 50 + up, 34, 50 + up], M.hood, { part: 'hood', bevel: 5 });
          // Складки ткани от лба к острию.
          p.poly([44, 15 + up, 46, 14 + up, 75 + tip, 26 + up + tip, 70, 25.5 + up], M.hoodDark, { part: 'hood', paint: true });
          p.poly([50, 20 + up, 51, 19 + up, 68, 28 + up, 66, 30 + up], M.hoodDark, { part: 'hood', paint: true });
