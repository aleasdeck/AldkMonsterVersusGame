...
import { SLOT_KIND_GLYPH, artifactChip, gearStatInfo, gearTypeIcon, perkLine, reachDots, slotKindTip, tierTip } from './components';
...
/** Ячейка сокета: чип, имя и число артефакта; пустая — пунктирная, со значком типа вместо имени. Тип сокета — цвет левой кромки и подсказка; словами — только в подсказке. */
function socketCell(inst: ArtifactInstance | null, kind: SlotKind, s: DerivedStats): HTMLElement {
  if (!inst) return h('div', { class: `sock empty k-${kind}`, tip: slotKindTip(kind) }, artifactChip(null), h('span', { class: 'sock-name sock-kind' }, SLOT_KIND_GLYPH[kind]));
  const def = artifactDef(inst.id);
...
export function gearTile(gear: GearInstance, def: HeroDef, s: DerivedStats, opts: GearTileOpts = {}): HTMLElement {
  ...
  return h(
    ...
    perk,
    opts.expanded
      ? h('div', { class: 'art-list' }, ...gear.slots.map((a, i) => (a ? artifactRow(a, s) : h('div', { class: `art-row empty k-${slotKindAt(gear, i)}`, tip: slotKindTip(slotKindAt(gear, i)) }, artifactChip(null), h('span', { class: 'sock-kind' }, SLOT_KIND_GLYPH[slotKindAt(gear, i)]), h('span', { class: 'dim' }, 'свободный сокет')))))
