...
export interface DerivedStats {
  ...
  perDebuff: number;
  /** Урон первому в ряду (или цели приёма) после каждого приёма или заклинания героя, бесплатно («Цепная атака»). */
  chainDmg: number;
...
export type Effect =
  ...
  /** Финишер: урон = per × число атак героя в этом ходу, без кубика и усталости; счётчик атак не растёт. Без атак недоступен. */
  | { type: 'finisher'; per: number; target: 'enemy' }
