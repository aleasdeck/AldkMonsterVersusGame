...
export type StatusId =
  ...
  | 'stealth' // враги не видят героя; любая атака — удар в спину (крит мимо блока) и снимает статус (только герой)
  ...
  | 'cold' // холод (v0.47): копится; на COLD_FREEZE превращается в Оцепенение и обнуляется (только враги)
...
export interface DerivedStats {
  ...
  riposte: number;
  // ── Связки (v0.38) ──
  /** Доля (0..1), на которую удар оружием сильнее по кровоточащей цели («Кровавый след»; до v0.43 — плоская прибавка). */
  vsBleed: number;
  ...
  skillMp: number;
  // ── Архетипы и наборы (v0.43) ──
  /** Прибавка к силе каждого Кровотечения, которое вешает герой (набор «Кровь» 2/3). */
  bleedAdd: number;
  /** Доля сверх силы Кровотечения от героя: 0.5 — в полтора раза, округление вверх («Клятва крови»). */
  bleedMult: number;
  /** >0 — Кровотечение на врагах тикает ещё раз перед ходом героя (набор «Кровь» 3/3). */
  bleedTwice: number;
  /** Прибавка к силе каждого Горения, которое вешает герой (набор «Огонь» 2/3). */
  burnAdd: number;
  /** >0 — погибший горящий враг поджигает остальных своим Горением (набор «Огонь» 3/3). */
  burnSpread: number;
  /** Доля к урону ударов оружием: −0.3 — на треть слабее (ключевые вещи «Клятва крови», «Пироман»). Складывается, не ниже −0.9. */
  strikeMult: number;
  /** >0 — Горение на герое не держится («Жаропрочность»). */
  burnImmune: number;
  /** Блок в начале хода за каждого горящего врага («Жаропрочность»). */
  ...
  ambushStun: number;
  // ── Архетипы v0.47 (docs/plan-reworka.md §2.2) ──
  /** +N к каждому Яду, который вешает герой (набор «Яд» 2). */
  poisonAdd: number;
  /** >0 — Яд героя не спадает по сроку («Токсиколог»). */
  poisonNoDecay: number;
  /** Доля, на которую слабее удары отравленного врага (набор «Яд» 3). */
  poisonWeaken: number;
  /** +N к блоку каждого приёма, дающего блок: «Защититься», приёмы с блоком, Щитовой удар (набор «Щит» 2). */
  blockSkillAdd: number;
  /** Доля текущего блока, прибавляемая к удару оружием (набор «Щит» 3). */
  blockToDmg: number;
  /** Доля к максимуму HP (минус ключевых вещей «Бастион», «Обет»). */
  maxHpPct: number;
  /** >0 — Шипы героя бьют всех врагов, а не только ударившего (набор «Возмездие» 3). */
  thornsAll: number;
  /** Сила за каждый удар, дошедший до HP героя, до конца боя («Мученик»). */
  hitStr: number;
  /** >0 — лечение не действует («Мученик»). */
  noHeal: number;
  /** +N к каждому лечению героя (набор «Свет» 2). */
  healAdd: number;
  /** Доля сверх к каждому лечению («Обет»: +1 — вдвое). */
  healMult: number;
  /** >0 — каждое лечение героя бьёт первого врага на столько же (набор «Свет» 3). */
  healSmite: number;
  /** +N к удару оружием за каждый уже сделанный в этом ходу удар («Разгон»). */
  momentum: number;
  /** >0 — «Защититься» недоступно («Безрассудство»). */
  noDefend: number;
  /** >0 — каждый третий удар оружием в ходу возвращает стамину (набор «Серия» 3). */
  thirdFree: number;
  /** >0 — крит только наверняка: из тени, по оглушённым, «Верный глаз» и прочие гарантированные («Хладнокровие»). */
  critOnlySure: number;
  /** STA за крит, раз в ход (набор «Тень» 3). */
  critSta: number;
  /** Сколько ударов оружием за ход вешают Холод 1 («Ледяной клинок»). */
  onHitCold: number;
  /** +N к каждому Холоду героя (набор «Холод» 2). */
  coldAdd: number;
  /** >0 — Оцепенение держит два хода, удары по оцепеневшему слабее на 30 % («Вечная мерзлота»). */
  frozenLong: number;
...
export type Effect =
  | {
      ...
      /** Око за око (v0.47): + доля урона, полученного героем за прошлый ход врагов. */
      revenge?: number;
      /** Кара (v0.47): + лечение героя за этот ход × smite. */
      smite?: number;
      /** Раскол (v0.47): по оцепеневшей цели удар ×vsFrozen и Оцепенение снимается. */
      vsFrozen?: number;
    }
  /**
  ...
  /** Заражение: перечисленные статусы с цели копируются на всех остальных врагов (сила и срок те же, стакаются как обычно); pct — доля силы (Кровавая баня). */
  | { type: 'spread'; statuses: StatusId[]; target: 'enemy'; pct?: number }
  /** Испепеление (v0.43): урон = Горение цели × mult, как рана — мимо блока и уворота; Горение остаётся. */
  | { type: 'scorch'; mult: number; target: 'enemy' }
  ...
  /** Катализатор (v0.47): статус на цели ×mult; без статуса приём недоступен. */
  | { type: 'amplify'; status: StatusId; mult: number; target: 'enemy' }
  /** Обвал щита (v0.47): весь блок героя уходит, каждый враг получает pct потраченного — как удар, без кубика, усталости и крита. */
  | { type: 'blockBurst'; pct: number }
...
export interface ArtifactDef {
  ...
  fx?: FxSpec;
  /** Архетипы (v0.43): в чей набор вещь идёт. Без поля — общая вещь, в наборы не считается. */
  tags?: ArchetypeId[];
  /** Ключевая вещь архетипа (v0.43): ломает правило за цену, выпадает только тиром 1 и вдвое реже. */
  keystone?: boolean;
}
...
export interface HeroBattle extends Combatant {
  ...
  struckAny: boolean;
  /** Урон по HP героя за прошлый ход врагов и за нынешний — для «Ока за око» (v0.47). */
  takenLast: number;
  takenNow: number;
  /** Сколько лечения пришло в этом ходу героя (со сверхлечением) — для «Кары» (v0.47). */
  healedTurn: number;
}
...
export interface EnemyState extends Combatant {
  ...
  /** Множители под акт забега: HP/лечение и урон/DoT. Считаются при появлении. */
  hpMult: number;
  dmgMult: number;
  /** Множитель блока: акт без удвоения длины боя (v0.51.1) — щит меряется ударом героя, а не толщиной врага. */
  blockMult: number;
  ...
  fronted?: boolean;
  /** Сколько раз враг уже цепенел (v0.47): каждое следующее Оцепенение требует больше Холода. */
  freezes?: number;
}
