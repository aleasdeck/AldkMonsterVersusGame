...
npx vitest run                                       # тесты движка (~2 с, 203 теста)
...
src/engine/   чистая логика, без DOM, покрыта тестами
  ...
  combat.ts     бой: статусы, урон, союзники, действия героя, ИИ врагов, createBattle, describeAction/computeIntent; дальность (REACH_MODEL, SUMMON_FRONT, BACK_ROW, reachableEnemies/actionReach/canReach)
...
src/data/     типизированные таблицы; каждый файл экспортирует list-based Record + xxxDef(id)
  ...
  artifacts.ts  46 артефактов: пассивные / активные физ. (STA) / активные маг. (MP), тиры 1–3 через t(a,b,c); 6 персональных (по одному на героя)
...
- Дальность (v0.26): ближний бой достаёт только `state.enemies[0]` (модель `first`); `reachAny` в DerivedStats — свойство типа оружия и базы копья (`Base.reach`), владение не участвует; `ArtifactDef.reach` — своя дальность приёма (без поля: `school: 'magic'` — любая цель, физический — как оружие). Проверка — `canReach`/`canUseAction` («Только первый в ряду»), UI показывает причину и тушит недосягаемого врага. Призванные и отделившиеся встают вперёд (`SUMMON_FRONT`, unshift в `placeEnemy`), союзники дальности не держат. Модель `rows` + `BACK_ROW` + `BACK_ROW_LAST` — запасные варианты до решения. Новый приём по врагу обязан решить свою дальность (щит и порез — в упор, склянка — любая), а тест, который бьёт второго врага ближним оружием, упадёт с этой причиной.
...
v0.26 (дальность, прототип на ветке) на 600 забегах: строго «первый в ряду» с призывом в хвост роняет ближних героев на 8–11 пунктов (Воин 33.5 %, Ассасин 39.7 %), с призывом вперёд — на 2–5 (Воин 36–37 %, Ассасин 44–45 %, Паладин и Берсерк 53 %), два ряда (`rows`) — бесплатно. Рычаги компенсации ближнему оружию: +1 Сила свойством типа — перебор (+10…+15 пунктов, Берсерк 65 %), урон врагов 1.2 / 1.4 / 1.5 — все в коридоре (Воин 47.5 %, Паладин 60 %). Статические надбавки боту за дальность вредят (3–8 пунктов) — бот видит дальность перебором ходов, ценить её в статике не надо. Полные цифры — GDD §13. В облачной сессии лаборатория — `runlab.py` в scratchpad: копия `src`+`tests`+конфигов на вариант, патчи констант строками, четыре варианта параллельно на четырёх ядрах, 600 забегов × 6 героев ≈ 2 мин на вариант.
