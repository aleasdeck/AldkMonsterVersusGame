...
npx vitest run                                       # тесты движка (~3 с, 225 тестов)
...
src/data/     типизированные таблицы; каждый файл экспортирует list-based Record + xxxDef(id)
  ...
  enemies.ts    69 врагов по локациям (секции ═══; два гнома-вора числятся за лесом, но приходят событием любого акта), хелпер act(), ai: cycle | boss-rules; evade — процентный уворот с порога, spriteScale — размер спрайта
...
Единственный источник истины — умный бот (`tests/sim/bot.ts`), цель **30–50 % побед за каждого героя** на 300 забегах (с v0.26.1, решение пользователя: живой игрок играет лучше бота, так что коридор бота должен быть ниже; до этого 40–60 % — исторические записи ниже и в GDD §13 сравнивают с тем коридором). Живой игрок при 50 % бота сложность чувствует; при 90 % — нет.
