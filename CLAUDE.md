...
npx vitest run                                       # тесты движка (~2 с, 179 тестов)
...
src/data/     типизированные таблицы; каждый файл экспортирует list-based Record + xxxDef(id)
  ...
  artifacts.ts  45 артефактов: пассивные / активные физ. (STA) / активные маг. (MP), тиры 1–3 через t(a,b,c); 6 персональных (по одному на героя)
